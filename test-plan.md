...
| API-074 | Workspace GET with session subject ≠ path participant       | 403 subject mismatch           | [covered] | `index.test.ts` "rejects a ProjectFlow workspace request whose study session names a different participant" — signs a real study-session token for one participant, requests a second participant's workspace path over a signed non-bypassed origin (`https://darwin-api.example`), and asserts 403 `study_session_subject_mismatch` |
...
| GH-005 | Path traversal in a context path rejected; `baseSha` regex; branch encoded                | rejected                        | [covered] | `github-source.test.ts`: "rejects a context path that escapes the repository via a nested .. segment", "rejects a bare .. context path", "rejects a percent-encoded %2e%2e context path", "rejects a context path with a leading slash" (traversal strings as `contextPath`); "rejects an invalid commitSha override that fails the baseSha format" and "rejects a malformed sha returned by the GitHub commit lookup" (`baseSha` regex, both call sites); "encodes the branch name before composing the GitHub commit lookup URL" (`encodeURIComponent(branch)`) |
| GH-006 | GitHub fetch beyond the timeout is aborted                                                 | timeout signal                  | [covered] | `github-source.test.ts` "bounds GitHub request duration" proves the abort mechanism via an injected `requestTimeoutMs: 5` override; "applies the production GitHub request timeout default when no override is supplied" proves the production default (`GITHUB_REQUEST_TIMEOUT_MS = 10_000`) is what is wired in when the override is omitted, using fake timers to assert it has not fired at 9,999ms and has fired by 10,000ms |
...
| SEC-041 | Workspace IDOR guard: token for participant A on participant B's path → 403                        | rejected                     | [covered] | `index.test.ts` "rejects a ProjectFlow workspace request whose study session names a different participant" — same test as API-074, cross-referenced here |
...
| Prior fixed — workspace IDOR (S1)        | SEC-041, API-074    | **[covered]** — `index.test.ts` "rejects a ProjectFlow workspace request whose study session names a different participant" signs a study-session token for one participant and exercises a second participant's path over a non-bypassed origin |
